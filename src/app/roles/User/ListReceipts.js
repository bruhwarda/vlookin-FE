...
export const ListReceipts = () => {
    ...
    useEffect(() => {
        setLoading(true)
        axios.get(apiRoutes.getReceipts)
            .then((res) => {
    ...
    return (
        <div>
            <SideBar children={<CusTable columns={columns} data={data} heading={'View Receipts'} subHeading={'tenant panel'} loading={loading} route={routePaths.Admin.login} showDrawer={showDrawer}  searchQuery={searchQuery} setSearchQuery={setSearchQuery} />} showDrawer={showDrawer} open={open} setOpen={setOpen} items={items} />
            <CustomAlert />
        </div>
    )
}